...
const changeCurrentPassword = asyncHandler(async (req, res) => {
  ...
  //When we want to confirm new password with confirm password adding confirm password
  // if (!(newPassword === confPasssword))
  //   throw new ApiError(400, "Password doesn't match");
  const user = await User.findById(req.user?._id);
  const isPasswordCprrect = await user.isPasswordCorrect(oldPassword);
  if (!isPasswordCprrect) throw new ApiError(400, "Invalid old password");
